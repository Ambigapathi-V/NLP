...
class DataTransformation:
    ...
    def imblance_data_cleaning(self):
        try:
            logging.info("Cleaning imbalanced data...")
            imblance_data = pd.read_csv(self.data_ingestion_artifacts.imblanced_data_path)
            # Strip any leading/trailing whitespace characters in column names
            imblance_data.columns = imblance_data.columns.str.strip()
            print(imblance_data.columns)
            print(imblance_data.head())
            imblance_data.drop(self.data_transformation_config.ID, axis=self.data_transformation_config.AXIS, inplace=self.data_transformation_config.INPLACE)
            return imblance_data
            logging.info("Cleaning imbalanced data completed.")
    ...
    def concat_dataframe(self):
        try:
            ...
            frame = [self.raw_data_cleaning(), self.imblance_data_cleaning()]
            df = pd.concat(frame)
            print(df.head())
